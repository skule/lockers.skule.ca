    ...
    if(gettype($_POST['lockerid']) !== "string" || gettype($_POST['expires']) !== "string" || gettype($_POST['capture']) !== "string" || gettype($_POST['order']) !== "string"){
      ...
      if(!isset($err)){
        ...
        if (strtotime($end) <= strtotime($start)) {
          ...
          $sql = "INSERT INTO `record` (record_start, record_end, record_price, student_email, locker_id, record_order_id, record_capture_id, book_date)
          VALUES ('$start', '$end', '$price', '$semail', '$lid', '$order_id', '$capture_id', '$time');";
          ...
          $msg = "<a href='index.php' class='white-text'><i class='fas fa-arrow-circle-left'></i></a> Booking success. Locker ID: ".htmlspecialchars($lid)."<br/>\nPlease save this page for your records. Transaction ID: <b>".htmlspecialchars($order_id)."-".htmlspecialchars($capture_id)."</b>";
          $msgClass = "green";
        }
      }
    }
